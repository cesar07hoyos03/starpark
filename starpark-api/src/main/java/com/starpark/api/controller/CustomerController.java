/**
 * Copyright 2017, Starpark
 * http://www.starpark.com
 * All rights reserved
 * @since 2017-10-20
 */
package com.starpark.api.controller;

import java.util.List;

import javax.validation.Valid;

import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.http.HttpStatus;
import org.springframework.http.ResponseEntity;
import org.springframework.stereotype.Controller;
import org.springframework.web.bind.annotation.GetMapping;
import org.springframework.web.bind.annotation.PostMapping;
import org.springframework.web.bind.annotation.RequestBody;
import org.springframework.web.bind.annotation.RequestMapping;

import com.starpark.api.constant.ResourceEndpoint;
import com.starpark.api.domain.Customer;
import com.starpark.api.service.CustomerService;

import lombok.extern.slf4j.Slf4j;

/**
 * Controller to receive request related with customer.
 * 
 * @since 0.0.1
 * @version 0.0.1
 * @author <a href="cesar07hoyos03@gmail.com">C&eacute;sar Hoyos</a>
 *
 */
@Controller
@RequestMapping(value = ResourceEndpoint.CUSTOMER_RESOURCE)
@Slf4j
public class CustomerController {

	@Autowired
	private CustomerService service;

	@GetMapping()
	public ResponseEntity<List<Customer>> getCustomers() {

		log.info("Received request for Get Customer");

		return new ResponseEntity<>(this.service.getCustomers(), HttpStatus.OK);
	}

	@PostMapping
	public ResponseEntity<Void> addCustomer(@RequestBody @Valid Customer customer) {
		this.service.addCustomer(customer);
		return new ResponseEntity<>(HttpStatus.OK);
	}

}
